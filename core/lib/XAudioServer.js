...
// Hack for embedding flash in Lively
XAudioServer.prototype.initializeFlashAudio = function () {
    var audioServer = this,
        swfMarkup = '<object type="application/x-shockwave-flash" style="" data="' + Config.rootPath + 'users/Dan/' + 'XAudioJS.swf" width="8" height="8" id="XAudioJS"><param name="allowscriptaccess" value="always" /></object>';

    function setupInMorphic() {
        alertOK('Applying XAudio Flash fix...');
	var swf = document.getElementById("XAudioJS");
        if (swf) swf.parentNode.removeChild(swf);

        var container = $('<div style="position: absolute; top: 4000px;" />');
        container.append(swfMarkup);
        $($world.renderContext().morphNode).append(container);
        audioServer.audioHandleFlash = document.getElementById("XAudioJS");
	audioServer.audioType = 2;
    }

    if (!window.$world) {
        window.setTimeout(function() {
            XAudioServer.prototype.initializeFlashAudio.call(audioServer);
        }, 100);
    } else {
        setupInMorphic();
    }
