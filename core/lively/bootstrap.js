        ...
        JSLoader = {
            ...
            loadJs: function (url, onLoadCb, loadSync, okToUseCache,
                ...
                if (this.scriptInDOM(url)) {
                    console.log(
                        'script ' + url + ' already loaded or loading'
                    );
                    return null;
                }
                ...
                var originalLoader = this,
                    combinedLoader = {
                        ...
                        expectToLoadModules: function (listOfRelativePaths) {
                            ...
                            for (i = 0; i < len; i++) {
                                var moduleName =
                                    listOfRelativePaths[i].replace(/\//g, '.');
            ...
            findParentScriptNode: function () {
                ...
                if (!node) {
                    throw new Error(
                        'Cannot find parent node for scripts'
                    );
                }
                return node;
            },
...
        var LivelyLoader;
        ...
        LivelyLoader = {
            ...
            // FIXME: Something is wrong with the lively-libs, use debug only
            // to activate loading on ios 5
            libsFile: (function () {
                // if (useMinifiedLibs) {
                //     return 'lib/lively-libs.js';
                // }
                return 'lib/lively-libs-debug.js';
            }()),
            ...
            modulePaths: (function setModulePaths() {
                ...
                var defaultPaths = [
                    'users/', 'projects/', 'documentation/', 'server/'
                ];
            ...
            installWatcher: function (target, propName, haltWhenChanged) {
                ...
                console.log(
                    'Watcher for ' + target + '.' + propName + ' installed'
                );
            },
        ...
        EmbededLoader = {
            ...
            embedAndLoadWorld: function (worldURL, targetElement) {
                ...
                if (canvas.getElementsByTagName('g').length > 0) {
                    ...
                    canvas.setAttribute(
                        "width",
                        worldElement.childNodes[0].width.baseVal.value
                            .toString() + 'px'
                    );
                    canvas.setAttribute(
                        "height",
                        worldElement.childNodes[0].height.baseVal.value
                            .toString() + 'px'
                    );
                } else {
                    // FIXME!!!
                    ...
                    canvas.setAttribute(
                        "width",
                        targetElement.clientWidth + 'px'
                    );
                    canvas.setAttribute(
                        "height",
                        targetElement.clientHeight + 'px'
                    );
            ...
            convertCDATASections: function (el) {
                ...
                if (el.nodeType === document.CDATA_SECTION_NODE) {
                    text = el.ownerDocument.createTextNode(el.data);
    ...
    (function setupLivelyMigrationSupport(Global) {
        ...
        LivelyMigrationSupport = {
            ...
            fixModuleName: function (name) {
                ...
                for (oldName in this.moduleRenameDict) {
                    // FIXME: no check of own property?
                    if (oldName === name) {
                        return this.moduleRenameDict[oldName];
                    }
                }
                return name;
            },
