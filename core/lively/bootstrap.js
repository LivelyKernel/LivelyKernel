...
(function bootstrapLively(Global) {
    ...
    var isFireBug = isFirefox
                 && window.console
                 && window.console.firebug !== undefined;
    var useMinifiedLibs = document.location.host.indexOf('localhost') === -1;

    function livelyConfigExists() {
        return typeof Global.Config !== "undefined";
    }
    ...
    (function setupConsole() {
        ...
        function addWrappers() {
            ...
            for (name in platformConsole) {
                // FIXME: no own property check?
                props.push(name);
            }
        ...
        function removeWrappers() {
            for (var name in platformConsole) {
                // FIXME: no own property check?
                if (name[0] !== '$') continue;
                var cleanName = name.substring(1, name.length);
                platformConsole[cleanName] = platformConsole[name];
    ...
    (function setupLoadingScreen(Global) {
        var LoadingScreen = {
            ...
            buildLoadingLogo: function () {
                ...
                logoAndText.setAttribute(
                  ...
                  + "width: 80px; height: 108px; background-color: white;");
            ...
            buildBrokenWorldMessage: function () {
                var el = document.createElement('div'),
                    ...
                    repairURL = Config.rootPath
                              + 'BrokenWorldRepairSite.xhtml?brokenWorldURL='
                ...
                el.setAttribute(
                  ...
                  + "white; font-family: Arial,times; color: red; "
                  + "font-size: large-x;");
            ...
            buildConsole: function () {
                ...
                console.setAttribute(
                    'style',
                    "position: absolute; top: 0px; font-family: monospace; "
                  + "color: rgb(0,255,64); font-size: medium; "
                  + "padding-bottom: 20px;");
                ...
                function addLine(str, style) {
                    ...
                    // html5 does not support cdata sections in the document.
                    // if the creation fails with a NOT_SUPPORTED_ERR, we
            ...
            buildConsoleButton: function () {
                ...
                a.setAttribute(
                    'style',
                    "position: fixed; right: 170px; top: 20px; width: 70px; "
            ...
            buildCloseButton: function () {
                ...
                a.setAttribute(
                    'style',
                    "position: fixed; right: 90px; top: 20px; width: 70px; "
                  + "text-align:center; font-family: monospace; "
                  + "border: 1px solid; border-color: rgb(100,100,100); "
                  + "color: rgb(100,100,100)");
    ...
    (function setupJSLoader(Global) {
        var JSLoader = {
            ...
            loadCombinedModules: function (combinedFileUrl, callback, hash) {
                ...
                // that are not included in the combined file, those will be
                // loaded as well.
                ...
                var originalLoader = this,
                    combinedLoader = {
                        ...
                        expectToLoadModules: function (listOfRelativePaths) {
                            ...
                            this.expectedModuleURLs = new Array(
                                    listOfRelativePaths.length
                            );
                            ...
                            for (i = 0; i < len; i++) {
                                var moduleName = listOfRelativePaths[i]
                                                    .replace(/\//g, '.');
                        ...
                        scriptInDOM: function (url) {
                            return (
                                originalLoader.scriptInDOM(url)
                    ...
                    callCallback = function () {
                        ...
                        // FIXME, Filter out the modules already loaded
                        var realModules = combinedLoader.expectedModules
                                            .select(
                            function (ea) {
                                // FIXME, better now throw error in
                                // Class.forName
                                return (
                        ...
                        Global.lively.require(realModules).toRun(callback);
            ...
            findParentScriptNode: function () {
                ...
                if (!node) {
                    throw new Error('Cannot find parent node for scripts');
                }
                return node;
            },
            ...
            resolveURLString: function (urlString) {
                ...
                // actually lively.Core should require lively.Network
                //   -- but lively.Network indirectly
            ...
            makeUncached: function (urlString, cacheQuery) {
                ...
                return (urlString + (urlString.indexOf('?') === -1 ? '?' : '&')
                        + cacheQuery
                );
            },
    ...
    (function setupLivelyLoader(Global) {
        ...
        var libsFile = /*useMinifiedLibs ? 'lib/lively-libs.js' :*/ 'lib/lively-libs-debug.js',
            ...
            LivelyLoader = {
                ...
                codeBase: (function findCodeBase() {
                    ...
                    if (!urlFound) {
                        console.warn(
                            'Cannot find codebase, have to guess...'
                        );
                    ...
                    var codeBase = Global.JSLoader.makeAbsolute(
                                        Global.JSLoader.dirOfURL(urlFound)
                                        + '../'
                                   );
                ...
                rootPath: (function findRootPath() {
                    ...
                    if (!urlFound) {
                        console.warn(
                            'Cannot find bootstrap.js, have to guess...'
                        );
                        return Global.JSLoader.dirOfURL(
                                    window.location.href.toString()
                               );
                ...
                modulePaths: (function setModulePaths() {
                    ...
                    if (Config.modulePaths === undefined) {
                        ...
                        Config.modulePaths = Config.modulePaths.concat(
                                                defaultPaths
                                             );
                    }
                ...
                installWatcher: function (target, propName, haltWhenChanged) {
                    ...
                    target.__defineSetter__(propName, function (v) {
                        ...
                        console.log(
                            target.toString() + '.' + propName
                            + ' changed: ' + v
                        );
                        // FIXME: debugger smell in terms of code conventions
                    ...
                    console.log(
                        'Watcher for ' + target + '.' + propName
                        + ' installed'
                    );
                },

                createConfigObject: function () {
                    // Should have addtional logic for the case when no no
                    // window object exist...
                    if (!window.Config) window.Config = {};
                ...
                loadMain: function (canvas, startupFunc) {
                    ...
                    require('lively.bindings', 'lively.Main').toRun(
                        function () {
                ...
                bootstrap: function (thenDoFunc) {
                    ...
                    var url = document.URL,
                        dontBootstrap = (
                            Config.standAlone
                            || url.indexOf('dontBootstrap=true')
                               >= 0
                        );
                    ...
                    if (optimizedLoading) {
                        ...
                        var hashUrl =
                                codeBase + 'generated/combinedModulesHash.txt',
                            combinedModulesUrl =
                                codeBase + 'generated/combinedModules.js',
                    ...
                    Global.JSLoader.resolveAndLoadAll(
                        codeBase, this.bootstrapFiles, thenDoFunc
                    );
                }
            };
        ...
        EmbededLoader = {
            ...
            embedAndLoadWorld: function (worldURL, targetElement) {
                ...
                if (canvas.getElementsByTagName('g').length > 0) {
                    // FIXME ugly hack: width/height not properly saved in
                    // canvas element so reset it to the width/height of the
                    // rect of the worldmorph
                    Config.resizeScreenToWorldBounds = false;
                    worldElement = canvas.getElementsByTagName('g')[0];
                    var morphElement = worldElement.childNodes[0];
                    canvas.setAttribute(
                        "width",
                        morphElement.width.baseVal.value.toString() + 'px');
                    canvas.setAttribute(
                        "height",
                        morphElement.height.baseVal.value.toString() + 'px');
                } else {
                    // FIXME!!!
                    Config.resizeScreenToWorldBounds = false;
                    canvas.setAttribute(
    ...
    (function setupLivelyMigrationSupport(Global) {
        ...
        Global.LivelyMigrationSupport = LivelyMigrationSupport;
    }(Global));
