...
'Fill', {
    fillDown: function(){
        ...
        for (i= 0; i< this.grid.arrSelectedCells.length; i++) {
           arrDisplay.push({x:this.grid.arrSelectedCells[i].gridPos().x.toString() ,y:this.grid.arrSelectedCells[i].gridPos().y})
        }
    ...
    imgFill_Click: function() {
        if (this.oFillMenu){
           ...
           var arrItems= [
            	...
            	['Fill Right',  function() { self.fillRight() }],
                ['Fill Up',  function() { self.fillUp() }],
                ['Fill Left',  function() { self.fillLeft() }]];
