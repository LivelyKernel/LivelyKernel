...
lively.BuildSpec('lively.net.tools.Lively2LivelyInspector', {
    ...
    _Position: lively.pt(646.0,249.5),
    ...
    .SessionList select {\n\
        ...
        submorphs: [{
            ...
            // show();
            // show(this.renderContext().subNodes[4])
            // var n = this.getItemIndexFromEventHTML(this.renderContext(), evt);
            // if (n === -1) return;
            // show(n + '...' + this.renderContext().subNodes[n].childNodes[0].textContent);
        }
        },{
        ...
        var stringified = morph.copy('true');
        var codeTemplate = 
    "    var json = '%s',\n"
    + "        morph = lively.persistence.Serializer.deserialize(json);\n"
    + "    if (morph) {\n"
    + "        morph.openInWorldCenter();\n"
    + "        alertOK('Got morph from %s');\n"
    + "        \"Morph usccessfully send to %s\";\n"
    + "    } else {\n"
    + "        \"Could not deserialize morph\";\n"
    + "    }"
        var code = Strings.format(codeTemplate, stringified.replace(/'/g, "\\'").replace(/\\n/g, "\\\\n"), sess.username, id);
        sess.remoteEval(id, code, function(msg) {
            alertOK(msg.data.error || msg.data.result);
        });
    },
