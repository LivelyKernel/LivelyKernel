...
// .collect(function(c) { var code = c.charCodeAt(0); return escapes[code] ? escapes[code] : c })
    ...
    downloadDataURI: function(options) {
        ...
        lively.$.isPlainObject(options) || (options = {data: options});
        ...
        lively.$('<form method="post" action="'+options.url+'" style="display:none"><input type="hidden" name="filename" value="'+options.filename+'"/><input type="hidden" name="data" value="'+options.data+'"/></form>').submit().remove();
    },
